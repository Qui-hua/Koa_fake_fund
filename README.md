...
# 基金買賣api
    ...
    主要透過使用者下單和基金交易所回傳成交資訊進行模擬。當使用者下單時，會檢查相關欄位（如該使用者銀行有無該基金幣別帳戶、基金帳戶代號是否有該用戶等），如欄位正確後會再檢查假銀行中的餘額，如當下餘額不足會發將相關資訊送至maillist，等待mail server進行發送。如下單成功後，也會將下單成功訊息發至maillist，等待mail server進行發送。
